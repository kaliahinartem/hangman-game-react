...
class Hangman extends Component {
  ...
  */
  handleGuess(evt) {
    ...
    let isGuessed = this.state.answer.includes(ltr);
    console.log(this.state.guessed);
    this.setState(st => ({
      guessed: st.guessed.add(ltr),
      nWrong: st.nWrong + (isGuessed ? 0 : 1),
    }));
  }
  ...
  render() {
    const isGameOver = this.state.nWrong >= this.props.maxWrong,
        altText = `${this.state.nWrong} wrong guesses, ${this.props.maxWrong - this.state.nWrong} attempts left`,
